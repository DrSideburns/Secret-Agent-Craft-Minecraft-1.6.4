...
public class EntityNinja extends EntityMob implements IRangedAttackMob {
	...
	public EntityNinja(World par1World) {
		...
		this.tasks.addTask(2, new EntityAIArrowAttack(this, 1.25D, 25, 10.0F));
    ...
    {
		this.dropItem(secretAgentMod.throwingKnife.itemID, 3);
    }

	 public void attackEntityWithRangedAttack(EntityLivingBase par1EntityLivingBase, float par2)
	    {
			 	EntityKunai entitykunai = new EntityKunai(this.worldObj, this);
			 	double d0 = par1EntityLivingBase.posX - this.posX;
		        double d1 = par1EntityLivingBase.posY + (double)par1EntityLivingBase.getEyeHeight() - 1.100000023841858D - entitykunai.posY;
		        double d2 = par1EntityLivingBase.posZ - this.posZ;
		        float f1 = MathHelper.sqrt_double(d0 * d0 + d2 * d2) * 0.2F;
		        entitykunai.setThrowableHeading(d0, d1 + (double)f1, d2, 1.6F, 12.0F);
		        this.playSound("random.bow", 1.0F, 1.0F / (this.getRNG().nextFloat() * 0.4F + 0.8F));
		        this.worldObj.spawnEntityInWorld(entitykunai);
	        }
